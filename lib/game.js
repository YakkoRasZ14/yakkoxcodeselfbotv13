...
const cekWaktuTG = (keysa, _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            keysa.sendMessage(_dir[position].id, `*Waktu habis*\n\n*Jawaban :* ${_dir[position].jawaban}`, MessageType.text)
...
const cekWaktuFam = (keysa, _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            ...
            keysa.sendMessage(_dir[position].id, juwu, MessageType.text)
            _dir.splice(position, 1)
        }
    }, 1000)
}
